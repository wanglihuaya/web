...
ajax("./data/index.json", "get", function (res) {
  ...
  var hot = res.hot;
  console.log(hot); //使用数据动态渲染页面  使用模板引擎
  //3.初始化模板

  var html = template("sele", hot); //hot  { content:[]   selector:[{},{}]}
  //将初始化好的html字符串 渲染进页面

  liveR.innerHTML = html;
  var html1 = template("con", hot);
  liveL.innerHTML = html1; //添加选项卡效果  给每一个选项li 添加鼠标移入事件  移入谁 就让下标相同的内容li显示出来 其他li隐藏
